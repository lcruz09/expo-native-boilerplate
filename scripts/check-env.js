...
envFiles.forEach((file) => {
  ...
  if (exists) {
    ...
    try {
      ...
      const expoPublicVars = lines.filter((line) =>
        line.includes("EXPO_PUBLIC_"),
      );
      ...
      lines.forEach((line, index) => {
        ...
        if (!line.includes("=")) {
          ...
          const hasSpaces =
            parts[0] !== key || (parts[1] && parts[1] !== parts[1].trim());

          if (hasSpaces) {
            console.log(
              `     ⚠️  Line ${lineNum}: Has spaces around '=' (should be KEY=value)`,
            );
          }

          if (!key.startsWith("EXPO_PUBLIC_")) {
            console.log(
              `     ⚠️  Line ${lineNum}: "${key}" doesn't start with EXPO_PUBLIC_`,
            );
          }
          ...
          if (
            ...
            console.log(
              `     ℹ️  Line ${lineNum}: Value is quoted (usually not needed)`,
            );
          }
        }
      });
...
if (envVars.length === 0) {
  ...
  console.log(
    "  ℹ️  This is expected - Expo loads them at runtime, not in Node.js scripts",
  );
} else {
  envVars.forEach((key) => {
...
console.log("\n💡 Recommendations:");
console.log(
  "  1. Ensure your .env file is in the project root (same level as package.json)",
);
